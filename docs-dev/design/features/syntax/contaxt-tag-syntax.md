...
| 標籤名稱 | 用途 | 生命週期 | 內容型態 | 有點字 | 單列 |
| --- |  --- |  --- |  --- |  --- |  --- |
| <數學> | 指定某明眼字區塊內的文字優先使用數學點字轉換器進行轉換工作。 | DuringConvertion | 數字（次數） | False | False |
| <縮排> | 指定某區塊內的文字要縮排一次。 | DuringConvertion | 數字（次數） | False | False |
| <標題> | 指定從這個標籤開始的頁標題。 | Forever | 文字 | True | True |
| <標題> | 指定從這個標籤開始的頁標題。 | Forever | 文字 | True | True |
| <座標> | 　 | DuringConvertion | 文字 | True | False |
| <分數> | 　 | DuringConvertion | 文字 | True | False |
| <時間> | 　 | DuringConvertion | 文字 | True | False |
| <表格> | 　 | DuringConvertion | 文字 | True | False |
| <刪> | 表示一段刪除的文字。例如: `<刪>▲abc▼</刪>`。特別注意此標籤有附帶額外的實心正反三角形各一，因為刪除的起時與結束都有不同的點字來表示，因此選擇正反時先三角形來作為轉點字的對應符號。 | DuringConvertion | 文字 | True | False |
