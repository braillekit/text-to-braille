...
轉成點字後，`BeginLineIndex` 會是 3（指向 "ccc" 那一列）。可是如果使用者在雙視編輯時，刪除了 "bbb" 那一列，那麼 `BeginLineIndex` 就會指到 "ddd" 去了。

為了確保標題列能永遠正確地「黏住」標題列，必須再用一個 `BrailleLine` 物件參考記住標題列下方的那一列（`BeginLine`，稱為「標題起始列」）。`BeginLineIndex` 和 `BeginLine` 必須交互確認與修正，以確保能夠取到正確的標題列。
